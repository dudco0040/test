...
def validation(model, criterion, evaluation_loader, opt, self_training=False): #converter
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        cost = criterion(preds, target) #preds.contiguous().view(-1, preds.shape[-1]), target.contiguous().view(-1)
...
def test(opt):
    ...
    opt.num_class = 4 #len(converter.character)
